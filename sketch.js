...
function preload() {
  mainFont = loadFont('Roboto-Regular.ttf');
}
...
function mousePressed() {
  ...
  // Si el panel está abierto, deja los controles al modal
  if (showPanel) return;
  if (!isTouchDevice) checkHoveredBirthday(mouseX, mouseY);
}

function keyPressed() {
  // Atajo UX: esc para cerrar panel
...
function createPanelElements() {
  ...
  inputName.style('width', '95%'); inputName.style('margin-bottom', '11px');
  inputName.attribute('placeholder', 'Nombre completo');
  inputName.style('font-size', '1.1em'); inputName.style('border-radius', '7px');
  inputName.style('border', '1.5px solid #e0e0e0');

  inputInitials = createInput('').parent(panelCard);
  inputInitials.style('width', '95%'); inputInitials.style('margin-bottom', '11px');
  inputInitials.attribute('placeholder', 'Iniciales');
  inputInitials.style('font-size', '1.1em'); inputInitials.style('border-radius', '7px');
  inputInitials.style('border', '1.5px solid #e0e0e0');

  inputDate = createInput('', 'date').parent(panelCard);
  inputDate.style('width', '95%'); inputDate.style('margin-bottom', '8px');
  inputDate.attribute('placeholder', 'Fecha');
  inputDate.style('font-size', '1.1em'); inputDate.style('border-radius', '7px');
  ...
  inputApprox.style('transform', 'scale(1.15)');

  addButton = createButton('Añadir cumpleaños').parent(panelCard);
  addButton.style('width', '100%'); addButton.style('height', '37px');
  addButton.style('font-size', '1.08em'); addButton.style('margin-top', '5px');
  addButton.style('border-radius', '7px'); addButton.style('border', 'none');
  addButton.style('background', 'linear-gradient(90deg,#e1fae7 30%, #d8e9fa 95%)');
  addButton.style('font-weight', '600'); addButton.style('color', '#245');
  addButton.mousePressed(addOrUpdateBirthday);

  cancelButton = createButton('×').parent(panelCard);
  cancelButton.style('position', 'absolute');
  cancelButton.style('top', '12px'); cancelButton.style('right', '22px');
...
function refreshListView() {
  ...
  birthdays.forEach((p, i) => {
    ...
    d.mouseOver(() => { d.style('background', 'rgba(220,236,240,0.13)'); });
    d.mouseOut(() => { d.style('background', ''); });

    let t = (p.name || "~") + " (" + (p.initials || "~") + ") " + (p.birthdate || "~") + (p.approximate ? " ~" : "");
    let txt = createSpan(t).parent(d);
    txt.style('flex', '1');
    txt.style('font-size', '1em');

    let editBtn = createButton("✎").parent(d);
    editBtn.style('font-size','1.1em'); editBtn.style('margin-right','11px');
    editBtn.style('border','none'); editBtn.style('background','none');
    editBtn.style('color','#2F446A'); editBtn.style('cursor','pointer');
    editBtn.mousePressed(() => { startEdit(i); });

    let removeBtn = createButton("🗑").parent(d);
    removeBtn.style('font-size','1.08em'); removeBtn.style('border','none');
    removeBtn.style('background','none'); removeBtn.style('color','crimson');
    removeBtn.style('cursor','pointer');
    removeBtn.mousePressed(() => { if (confirm("¿Borrar?")) deleteEntry(i); });
  });
}
...
function resetForm() {
  ...
  editingIndex = null;
  addButton.html('Añadir cumpleaños');
}
...
function getSeasonColor(date) {
  ...
  if ((month === 3 && day >= 20) || month === 4 || month === 5 || (month === 6 && day <= 20)) return color('#E1FAC8');
  if ((month === 6 && day >= 21) || month === 7 || month === 8 || (month === 9 && day <= 22)) return color('#F7F0C6');
  if ((month === 9 && day >= 23) || month === 10 || month === 11 || (month === 12 && day <= 20)) return color('#FFDEDE');
  return color('#C7F4F4');
}
...
function getChineseElement(birthDate, approximate) {
  ...
  const elements = ["Madera", "Madera", "Fuego", "Fuego", "Tierra", "Tierra", "Metal", "Metal", "Agua", "Agua"];
...
function drawBirthdays() {
  ...
  for (const [key, group] of Object.entries(birthdayGroups)) {
    ...
    for (let idx = 0; idx < group.length; idx++) {
      ...
      const isBirthdayToday = !approximate && realDate &&
...
function drawHoverInfo() {
  if (hoveredBirthday) {
    ...
    if (isTouchDevice) {
      ...
      infoX = constrain(mouseX - infoWidth / 2, padding, width - infoWidth - padding);
      infoY = constrain(mouseY - infoHeight - padding, padding, height - infoHeight - padding);
    }
    fill(0, 0, 100, 0.85);
    stroke(0, 0, 80);
    ...
    textSize(16);
    textStyle(BOLD);
    text((hoveredBirthday.name || hoveredBirthday.initials) + (approximate ? ' ~' : ''), infoX + infoWidth / 2, textY);

    textY += 23;
    textSize(14);
    textStyle(NORMAL);
    text(`Nacimiento: ${(hoveredBirthday.birthdate || '~')}${approximate ? ' ~' : ''}`, infoX + infoWidth / 2, textY);
    ...
    textY += 23;
    text(`Signo Chino: ${chineseZodiac} (${chineseElement})`, infoX + infoWidth / 2, textY);
